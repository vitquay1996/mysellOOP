...
<body> 
	...
	<div id="SubProductRoad">
		<div class="road">
			...
			$Cat = new Category();
			$Cat2 = new Category();
			$Cat = $Cat->findByPost($_GET['id']);
			$fullID = $Cat->id2;
			$id = [];
			$cutlength = -4;
			$iter = 0;
			while ((strlen($fullID) + $cutlength) >= 0) {
				$id[$iter] = substr($fullID, 0, $cutlength);
				$iter++;
				$cutlength-=4;
			}
			?>
			<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
			<a class="lv1" href="index.php">Home</a>
			<?php 
			for ($i = sizeof($id)-1; $i>=0; $i--) {
				$Cat2 = $Cat2->findById2($id[$i]);
				?>
				<a class="lv1 arrow" href="index.php?cat=<?php echo $Cat2->id2;?>"><?php echo $Cat2->name;?></a>
			<?php } ?>
				<a class="lv1 arrow" href="index.php?cat=<?php echo $Cat->id2;?>"><?php echo $Cat->name;?></a>
			</div>
		</div>
	</br>
	<div class="col-md-11">
